...
;(function(window, undefined) {
  ...
  var each = function (collection, callback, thisArg) {
    ...
    if (typeof length == 'number') {  
      while (++index < length) {
        if (callback(iteratee[index], index, collection) === false) return result
      }
    }
    else {  
  ...
  var assign = function (object, source, guard) {
    var index, iteratee = object, result = object;
  ...
  var defaults = function (object, source, guard) {
    ...
    for (var argsIndex = 1, argsLength = typeof guard == 'number' ? 2 : arguments.length; argsIndex < argsLength; argsIndex++) {
      ...
      for (index in iteratee) {
        if (hasOwnProperty.call(iteratee, index)) {    
        if (result[index] == null) result[index] = iteratee[index];    
        }
      }    
      }
    };
